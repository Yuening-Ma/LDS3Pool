...
def extract_bdsl_data(root_dir, date):
    ...
    for param_key, seeds in param_groups.items():
        model_dataset, pooling_method, dim_reduction = param_key
        ...
        # 将参数组合数据添加到结果中
        param_str = f"{model_dataset}_{pooling_method}_{dim_reduction}"
        results.append({
            "exp_type": "BDSL",
...
def extract_base_data(root_dir, date):
    """提取基准数据"""
    # 读取配置文件
    json_path = f'{root_dir}para_base_2025{date}.json'
    with open(json_path, 'r') as f:
        base_combinations = json.load(f)
    ...
    # 识别参数组合
    for config in base_combinations:
        # 提取除random_seed和index外的参数
        param_key = (
            config["model_dataset"],
            config["pooling_method"]
    ...
    # 处理每个参数组合
    exp_dir = os.path.join(root_dir, "BASE")
    for param_key, seeds in param_groups.items():
        model_dataset, pooling_method = param_key
        ...
        # 将参数组合数据添加到结果中
        param_str = f"{model_dataset}_{pooling_method}"
        results.append({
            "exp_type": "BASE",
            "param_combination": param_str,
            "model_dataset": model_dataset,
...
def save_expanded_data(data, output_file):
    ...
    # 创建表头
    headers = ['exp_type', 'param_combination', 'model_dataset', 'pooling_method', 'dim_reduction']
...
def main():
    ...
    # 提取基准数据
    base_data = extract_base_data(root_dir, date)
    
    # 合并数据
    combined_data = base_data + bdsl_data
    
    # 保存到root_dir目录下
    output_file = os.path.join(root_dir, f"extracted_data_{date}.csv")
    save_to_csv(combined_data, output_file)
    
    # 保存展开数据格式（每个随机种子对应单独的列）
    expanded_output_file = os.path.join(root_dir, f"extracted_data_expanded_{date}.csv")
    save_expanded_data(combined_data, expanded_output_file)
    
    # 输出提取的数据摘要
    print(f"合并数据: BDSL实验 {len(bdsl_data)}个参数组合 + 基准实验 {len(base_data)}个参数组合")
    print(f"总共: {len(combined_data)}个参数组合的数据")
